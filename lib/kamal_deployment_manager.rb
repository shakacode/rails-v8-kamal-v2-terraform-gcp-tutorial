...
class KamalDeploymentManager
  ...
  def print_ssl_note
    ssl_enabled = @config.dig("proxy", "ssl") != false
    return unless ssl_enabled

    yellow = "\e[33m"
    reset = "\e[0m"
    puts ""
    puts "#{yellow}💡 SSL note: If Chrome shows ERR_CERTIFICATE_TRANSPARENCY_REQUIRED, wait 5-10 minutes.#{reset}"
    puts "#{yellow}   The Let's Encrypt certificate was just issued and CT logs need time to propagate.#{reset}"
    puts "#{yellow}   Try an incognito window first — Chrome caches SSL state aggressively.#{reset}"
    puts "#{yellow}   This only happens on fresh deployments. Subsequent `kamal deploy` reuses the certificate.#{reset}"
    puts "#{yellow}   See docs/troubleshooting.md > SSL Certificate Issues for details.#{reset}"
  end
